...
export default function MessagesSection() {
  ...
  const handleToggleRead = async (id, read) => {
    ...
    if (result && result.ok === false) toast('error', 'Échec : API inaccessible');
  };

  const handleRemove = async (id) => {
    const result = await removeMessage(id);
    if (result && result.ok === false) toast('error', 'Échec de la suppression : API inaccessible');
  };

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl md:text-3xl font-extrabold text-black dark:text-white">Messages</h1>
      ...
      {error && <ErrorBanner message="API inaccessible : lecture des messages impossible." />}

      {!loading && !error && messages.length === 0 ? (
        <div className="card-root p-12 text-center">
          <FiInbox className="mx-auto w-10 h-10 text-primary/40" />
          <p className="mt-4 text-body dark:text-body-dark">Aucun message pour le moment.</p>
        </div>
      ) : (
