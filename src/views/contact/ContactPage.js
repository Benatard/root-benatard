...
export default function ContactPage() {
  ...
  return (
    <div>
      <PageMeta title="Contact" description={profile ? `Contactez ${profile.name} pour discuter de votre projet de développement web.` : undefined} />
        ...
        <div className="relative py-16 md:py-19 border-b border-stroke/70 dark:border-[#2C303B]">
          <Reveal className="max-w-3xl">
            ...
            <h1 className="mt-6 text-4xl md:text-5xl font-extrabold tracking-tight leading-tight text-black dark:text-white">
              Parlons de votre{' '}
              <span className="bg-gradient-to-r from-primary to-blue-600 bg-clip-text text-transparent">projet</span>
            </h1>
            <p className="mt-6 text-base md:text-lg text-body dark:text-body-dark leading-relaxed">
              Une idée d'application, un site à créer, un projet à améliorer ? Racontez-moi votre
              besoin, je vous réponds sous 24h ouvrées.
            </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <Reveal>
              <div className="card-root p-8 md:p-10">
                <h2 className="text-2xl font-bold tracking-tight text-black dark:text-white">Envoyez-moi un message</h2>
                <form onSubmit={handleSubmit} className="mt-8 space-y-6">
                  <div>
                    <label className="label-root mb-2.5" htmlFor="contact-nom">Votre Nom</label>
                    <input id="contact-nom" name="nom" value={form.nom} onChange={handleChange} className="input-root" placeholder="Votre nom" />
                  </div>
                  <div>
                    <label className="label-root mb-2.5" htmlFor="contact-email">Votre Email</label>
                    <input id="contact-email" name="email" type="email" value={form.email} onChange={handleChange} className="input-root" placeholder="votre@email.com" />
                  </div>
                  <div>
                    <label className="label-root mb-2.5" htmlFor="contact-message">Votre Message</label>
                    <textarea id="contact-message" name="message" value={form.message} onChange={handleChange} rows="5" className="input-root" placeholder="Décrivez votre projet..."></textarea>
                  </div>
                  <button type="submit" disabled={loading} className="btn-primary-root w-full disabled:opacity-60">
                    <FiSend className="w-4 h-4" /> {loading ? 'Envoi...' : 'Envoyer le message'}
                  </button>
                </form>
              </div>
            </Reveal>
          ...
          <Reveal>
            <div className="relative overflow-hidden bg-gradient-to-r from-primary to-blue-600 px-8 py-12 md:px-16 text-center">
              ...
              <p className="relative mt-4 text-white/80 max-w-xl mx-auto">
                N'attendez plus : une simple idée peut devenir votre prochaine application.
              </p>
