...
export default function ProfileAside({ profile, compact = false }) {
  ...
  const contactRows = [
    {
      icon: FiMail,
      label: t("aside.contactEmail"),
      value: profile.email,
      href: `mailto:${profile.email}`,
    },
    {
      icon: FiPhone,
      label: t("aside.contactPhone"),
      value: profile.phone,
      href: `tel:${profile.phone.replace(/\s/g, "")}`,
    },
    {
      icon: FiMapPin,
      label: t("aside.contactLocation"),
      value: profile.location,
    },
    {
      icon: FiGlobe,
      label: t("aside.contactWebsite"),
      ...
      <>
        <div className="card-root p-5 flex items-center gap-5">
          ...
          <div className="min-w-0">
            <h2 className="text-lg font-extrabold text-black dark:text-white truncate">
              {profile.name}
            </h2>
            <p className="text-sm font-medium text-primary">{profile.role}</p>
            <p className="text-xs text-body dark:text-body-dark mt-0.5">
              {profile.level}
            </p>
      ...
      <aside className="space-y-6">
        <div className="card-root overflow-hidden">
          ...
          <div className="relative px-8 pb-8 -mt-14 text-center">
            ...
            <h2 className="mt-5 text-2xl font-extrabold tracking-tight text-black dark:text-white">
              {profile.name}
            </h2>
            <p className="mt-1.5 text-sm font-semibold text-primary">
              {profile.role}
            </p>
            <p className="mt-0.5 text-xs text-body dark:text-body-dark">
              {profile.level}
            </p>
            <span className="badge-root mt-5 bg-emerald-600">
              <FiCheckCircle className="w-3.5 h-3.5" /> {profile.availability}
            </span>
            <p className="mt-5 text-sm leading-relaxed text-body dark:text-body-dark">
              {profile.tagline}
            </p>
          </div>
        </div>
              ...
              const content = (
                <div className="flex items-start gap-4 group/row">
                  ...
                  <div className="min-w-0 pt-1.5">
                    <p className="text-[11px] font-semibold uppercase tracking-widest text-body dark:text-body-dark">
                      {label}
                    </p>
                    <p className="mt-1 text-sm font-semibold text-dark dark:text-white break-words">
                      {value}
                    </p>
                  </div>
                </div>
        ...
        <div className="card-root p-7 grid grid-cols-2 gap-4 text-center">
          {(profile.stats || []).map(({ number, label }) => (
            <div key={label} className="py-2">
              ...
              <div className="mt-1 text-[10px] font-semibold uppercase tracking-widest text-body dark:text-body-dark">
                {label}
              </div>
            </div>
          ))}
        ...
        <div className="card-root p-6">
          <p className="text-[11px] font-semibold uppercase tracking-widest text-body dark:text-body-dark text-center">
            {t("aside.findMe")}
          </p>
