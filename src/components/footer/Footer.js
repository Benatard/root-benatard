...
export default function Footer({ profile }) {
  ...
  const handleSubscribe = async (e) => {
    e.preventDefault();
    if (!email.trim()) {
      toast('warning', t('footer.emailRequired'));
      return;
    }
    setSubscribing(true);
    try {
    ...
    <footer className="relative bg-transparent">
      ...
      <div className="page-container pt-16 pb-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div>
            <h3 className="text-lg font-bold text-black dark:text-white">{t('footer.quickLinks')}</h3>
          ...
          <div>
            <h3 className="text-lg font-bold text-black dark:text-white">{t('footer.mySkills')}</h3>
            <ul className="mt-5 space-y-3 text-sm text-body dark:text-body-dark">
              <li>
              ...
              <li>
                <Link to="/competences" className="inline-flex items-center gap-2 hover:text-primary hover:translate-x-0.5 transition-all duration-300">
                  <span className="h-1 w-1 bg-primary/40" /> {t('footer.skillDatabases')}
                </Link>
              </li>
              <li>
                <Link to="/videos" className="inline-flex items-center gap-2 hover:text-primary hover:translate-x-0.5 transition-all duration-300">
                  <span className="h-1 w-1 bg-primary/40" /> {t('footer.skillTutorials')}
                </Link>
              </li>
            </ul>
          </div>
          <div>
            <h3 className="text-lg font-bold text-black dark:text-white">{t('footer.newsletter')}</h3>
            ...
            {subscribed ? (
              <p className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-emerald-600 dark:text-emerald-400">
                <FiCheckCircle className="w-4 h-4" /> {t('footer.newsletterConfirmed')}
              </p>
            ) : (
            ...
            <h3 className="mt-8 text-lg font-bold text-black dark:text-white">{t('footer.contact')}</h3>
